...
import NriPrelude
import Test.Internal (Fuzzer (Fuzzer))
...
int =
  Gen.frequency
    [ (30, Gen.integral (Range.linear (-50) 50)),
      (2, Gen.constant 0),
      (10, Gen.integral (Range.linear 0 (maxInt - minInt))),
      (10, Gen.integral (Range.linear (minInt - maxInt) 0))
    ]
    |> Fuzzer
...
intRange min_ max_ =
  Gen.integral (Range.linear min_ max_)
    |> Fuzzer
