...
hset :: Internal.Handler -> Text -> Text -> Text -> Task Internal.Error ()
hset handler key field val =
  Redis.ByteString.hset handler key field (toB val)
...
hgetall :: Internal.Handler -> Text -> Task Internal.Error [(Maybe Text, Maybe Text)]
hgetall handler key =
...
hmset :: Internal.Handler -> Text -> [(Text, Text)] -> Task Internal.Error ()
